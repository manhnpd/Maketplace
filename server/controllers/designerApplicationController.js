...
const createApplication = async (req, res) => {
  const { fullName, email, phone, specialties, portfolioUrl, portfolioFiles, bio } = req.body;
  ...
  res.json({
    success: true,
    data: { applicationId: data.id, status: data.status, createdAt: data.created_at },
  });
};
